...
/* filename : tasks_Nodo_1.c                                                  */
/* version  : 1                                                               */
/* date     : 31/05/2021                                                      */
/* author   : Jos� Manuel Vidarte Llera                                       */
/* description : Planificaci�n de tareas Nodo 1 que se comunican por bus CAN  */
/*                                                                            */
/******************************************************************************/
...
int tcomp_n1_task1 = 7;
int tcomp_n1_task2 = 10;
int tcomp_n1_task3 = 40;
...
CANIntHandler(void)
{
    ...
    if(ui32Status == CAN_INT_INTID_STATUS){
        ...
        // Marcar flag como que se ha confirmado el envio del mensaje inicial de sincronizaci�n
        MsgObj4_init = 1;
    }
...
//*****************************************************************************
void
start_sync_init(void)
{
    // Enviar mensaje a nodos 2 y 3 para sincronizar inicio
    uint32_t ui32MsgData_init = 0;

    CANMsgObject4_init.ui32MsgID = 0;
    CANMsgObject4_init.ui32MsgIDMask = 0;
    CANMsgObject4_init.ui32Flags = MSG_OBJ_TX_INT_ENABLE;
    CANMsgObject4_init.ui32MsgLen = sizeof(ui32MsgData_init);
    CANMsgObject4_init.pui8MsgData = (uint8_t *)&ui32MsgData_init;

    //UARTprintf("Nodo 1: Envio primer mensaje para sincronizar inicios.\n");

    CANMessageSet(CAN0_BASE, 4, &CANMsgObject4_init, MSG_OBJ_TYPE_TX);

    while(!MsgObj4_init){}

    //UARTprintf("Nodo 1: ACK del mensaje de sincronizaci�n.\n");
}
...
Void task1(UArg arg0, UArg arg1)
{
    for (;;) {
        ...
        CS (tcomp_n1_task1) ;
        //Enviar msj de continuaci�n de tarea 1 a nodo 2 (id 0x1002)
        uint32_t ui32MsgData_init = 0;
        ...
        //UARTprintf("Nodo 1: Envio msg a nodo 2 para que continue con la tarea 1 y me quedo esperando confirmaci�n.\n");
        ...
        //6UARTprintf("Nodo 1: Recibo mensaje de que el �ltimo nodo (3) ha acabado con el resto de la tarea 1.\n");
    }
}
...
Void task2(UArg arg0, UArg arg1)
{
    for (;;) {
        ...
        // Esperar a que el nodo 3 acabe de ejecutar su parte de la tarea 2 (msg con id 0x2001)
        CANMsgObject2_task2.ui32MsgID = 0x2001;
        ...
        //UARTprintf("Nodo 1: Recibo mensaje de que el nodo 3 ha acabado con la primera parte de la tarea 2.\n");

        CS (tcomp_n1_task2) ;

        //Enviar msj de continuaci�n de tarea 2 a nodo 2 (id 0x2002)
        uint32_t ui32MsgData_init = 0;
        ...
        //UARTprintf("Nodo 1: Envio msg a nodo 2 para que continue con la tarea 2.\n");
        CANMessageSet(CAN0_BASE, 2, &CANMsgObject2_task2, MSG_OBJ_TYPE_TX);

        while(!g_bRXFlag2){}
        g_bRXFlag2 = 0; // Limpiar flag de recepci�n msg2

        // Esperar a que el �ltimo nodo (2) acabe de ejecutar su parte de la tarea 2 (msg con id 0x2000)
        ...
        //UARTprintf("Nodo 1: Recibo mensaje de que el �ltimo nodo (2) ha acabado con el resto de la tarea 2.\n");
...
Void task3(UArg arg0, UArg arg1)
{
    for (;;) {
        ...
        // Esperar a que el nodo 3 acabe de ejecutar su parte de la tarea 3 (msg con id 0x3001)
        CANMsgObject1_task1.ui32MsgID = 0x3001;
        CANMsgObject1_task1.ui32MsgIDMask = 0xffff;
        CANMsgObject1_task1.ui32Flags = (MSG_OBJ_RX_INT_ENABLE | MSG_OBJ_USE_ID_FILTER | MSG_OBJ_EXTENDED_ID);
        CANMsgObject1_task1.ui32MsgLen = 8;
        ...
        //UARTprintf("Nodo 1: Recibo mensaje de que el nodo 3 ha acabado con su parte de la tarea 3.\n");

        CS (tcomp_n1_task3) ;

        //Enviar msj de fin de tarea 3 a nodos 2 y 3 (id 0x3000)
        uint32_t ui32MsgData_init = 0;
        ...
        //UARTprintf("Nodo 1: Envio msg a nodos 2 y 3 para que den por finalizada la tarea 3.\n");
        CANMessageSet(CAN0_BASE, 3, &CANMsgObject1_task1, MSG_OBJ_TYPE_TX);

        while(!g_bRXFlag3){}
        g_bRXFlag3 = 0; // Limpiar flag de recepci�n msg1
    }
}
